...
State> {
  ...
  getVerificationCode = () => {
    ...
    api.getLoginPhoneVerifyCode({ phone: this.state.phone }).then(json => {
      Toast.info('发送成功', 1)
      let timeout = 60;
      this.setState({
  ...
  comfirePwd = () => {
    let pwd = this.state.pwd, confirePwd = this.state.confirmPwd;
    if (pwd !== confirePwd) {
  ...
  render() {
    return (
      <View style={style.main}>
        <View style={[style.header, global.flex, global.justifyContentSpaceBetween,
        global.alignItemsCenter]}>
          <TouchableOpacity onPress={() => this.props.navigation.goBack()}>
            <Icon name="left" style={[style.headerLeft, global.fontSize16]}></Icon>
          </TouchableOpacity>
          ...
          <View style={style.inputList}>
            <View style={style.inputItem}>
              <InputItem clear value={this.state.phone}
                style={style.input} placeholder="手机号码" type="number"
                onChange={value => {
            ...
            <View style={style.inputItem}>
              <InputItem clear value={this.state.pwd}
                style={style.input} placeholder="6位以上新密码" type="password"
                onChange={value => {
            ...
            <View style={style.inputItem}>
              <InputItem clear value={this.state.confirmPwd}
                style={style.input} placeholder="再次确认密码" type="password"
                onChange={value => {
