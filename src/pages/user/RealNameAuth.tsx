...
import {
  ScrollView, Text, TouchableOpacity, View, Platform, PermissionsAndroid,
} from "react-native";
import {
...
import {
  GENDER, GENDER_ZH, TECHNICAL_TITLE, TECHNICAL_TITLE_ZH,
} from "@api/doctor";
...
State> {
  ...
  init = async () => {
    try {
      let regionCidMapAreaName: RegionCidMapAreaName = {}, region = [],
        hospitalDepartmentList = [], hospitalDepartmentMap = [];
      ...
      let { data: { list: hospitalDepartment } } = await hospitalApi.getDepartmentList({
        page: this.state.page,
        limit: this.state.limit, filter: this.state.filter
      })
      ...
      let { data: { list: hospitalDepartmentSymptom } } = await hospitalApi.getAllSymptomList({
        page: this.state.page,
        limit: this.state.limit, filter: this.state.filter
      })
  ...
  medicalPracticeCertificateChange = async (practisingCertificatePicIdList: any) => {
    ...
    api.uploadImg(practisingCertificatePicIdList[0]).then(json => {
      console.log(json)
    }).catch(err => {
      console.log(err);
    })
  ...
  qualificationCertificatePicIdChange = (qualificationCertificatePicIdList: any) => {
    let qualificationCertificatePicIdSelectable = qualificationCertificatePicIdList.
      length < 2;
    this.setState({
    ...
    return (
      <>
        <View style={style.main}>
          <ScrollView style={style.content} keyboardShouldPersistTaps="handled">
            ...
            <View style={style.form}>
              ...
              <View style={[style.formItem, style.formAvatar, global.flex, global.alignItemsCenter, global.justifyContentStart]}>
                ...
                />
                <Text style={[style.fromItemTitle_2, global.fontSize14, global.fontStyle]}
                  numberOfLines={2}>
                  正面照, 清晰度高, 半身照(胸部以上)</Text>
              </View>
              ...
              <View style={style.formItem}>
                <InputItem clear style={[style.input, global.fontStyle, global.fontSize14]}
                  value={this.state.idCardNo} placeholder="身份证号码" type="number"
                  onChange={idCardNo => {
              ...
              global.justifyContentSpaceBetween, global.alignItemsCenter]}>
                ...
                <Picker data={this.gender} style={style.picker} cols={1}
                  value={this.state.gender} triggerType="onPress"
                  onChange={gender => {
                    this.setState({
                      gender,
                    })
                ...
                >
                  ...
                  global.justifyContentEnd, global.alignItemsCenter,]} >
                    <Text style={[style.topItemTitle, global.fontStyle, global.fontSize14]}>
                      {this.state.gender.length === 0 ? '请选择' :
                        GENDER_ZH[this.state.gender]}</Text>
                    <Icon name="right" style={[style.inputIcon, global.fontSize16]} />
                  </TouchableOpacity>
                </Picker>
              </View>
              ...
              global.justifyContentSpaceBetween, global.alignItemsCenter]}>
                ...
                <Picker data={this.technicalTitle} style={style.picker} cols={1}
                  value={this.state.technicalTitle} triggerType="onPress"
                ...
                >
                  ...
                  global.justifyContentEnd, global.alignItemsCenter,]} >
                    <Text style={[style.topItemTitle, global.fontStyle, global.fontSize14]}>
                      {this.state.technicalTitle.length === 0 ? '请选择' :
                        TECHNICAL_TITLE_ZH[this.state.technicalTitle]}</Text>
                    <Icon name="right" style={[style.inputIcon, global.fontSize16]} />
                  </TouchableOpacity>
                </Picker>
              </View>
            ...
            <View style={style.Theme}>
              <Text style={[style.ThemeTitle_2, global.fontStyle, global.fontSize15]}>
                平台接受住院医师 ( 仅接受三甲医院 )、主治医师、副主任医师及主任医师进行入驻</Text>
            </View>
            <View style={style.form}>
              <View style={[style.formTitle, global.flex, global.alignItemsCenter]}>
                <Text style={style.formIcon}></Text>
              ...
              global.justifyContentSpaceBetween, global.alignItemsCenter]}>
                ...
                <Picker data={this.state.hospitalDepartment} style={style.picker} cols={1}
                  value={this.state.departmentId} triggerType="onPress"
                ...
                >
                  ...
                  global.justifyContentEnd, global.alignItemsCenter,]} >
                    <Text style={[style.topItemTitle, global.fontStyle, global.fontSize14]}>
                      {this.state.departmentId.length === 0 ? '请选择' :
                        this.state.hospitalDepartmentMap[this.state.departmentId]}</Text>
                    <Icon name="right" style={[style.inputIcon, global.fontSize16]} />
                  </TouchableOpacity>
                </Picker>
              </View>
              ...
              <View style={[style.adeptSymptomIdList, global.flex, global.flexWrap]}>
                {this.state.adeptSymptomIdList.map((v: any, k: number) => {
                  return (<Text key={k} style={[style.adeptSymptomIdItem, global.fontSize14,
                  global.fontStyle]}>{v.name}</Text>);
                })}
              </View>
            </View>
            <View style={style.Theme}>
            ...
            <View style={style.form}>
              <View style={[style.formTitle, global.flex, global.alignItemsCenter]}>
                <Text style={style.formIcon}></Text>
                <Text style={[style.formThem, global.fontStyle, global.fontSize14]}>
                  请上传 <Text style={[style.formImportant, global.fontStyle,
                  global.fontSize14]}> 医师执业证书</Text>(两张)
                </Text>
              </View>
              <View style={[style.formItem, style.formAvatar, global.flex, global.alignItemsCenter, global.justifyContentStart]}>
                <ImagePicker
            ...
            <View style={style.form}>
              <View style={[style.formTitle, global.flex, global.alignItemsCenter]}>
                ...
                <Text style={[style.formThem, global.fontStyle, global.fontSize14]}>
                  请上传 <Text style={[style.formImportant, global.fontStyle,
                  global.fontSize14]}> 医师资格证书</Text>(两张)
                </Text>
              </View>
              <View style={[style.formItem, style.formAvatar, global.flex, global.alignItemsCenter, global.justifyContentStart]}>
                <ImagePicker
            ...
            <View style={style.form}>
              <View style={[style.formTitle, global.flex, global.alignItemsCenter]}>
                ...
                <Text style={[style.formThem, global.fontStyle, global.fontSize14]}>
                  请上传 <Text style={[style.formImportant, global.fontStyle,
                  global.fontSize14]}> 专业技术资格证书</Text>即职称证书(选填, 最多9张)
                </Text>
              </View>
          ...
          {/* 擅长选择 */}
          <View style={this.state.selectAdeptSymptomActive ? style.adeptSymptom :
            global.hidden}>
            ...
            <ScrollView style={style.adeptSymptomContent}>
              <View style={[style.formTitle, global.flex, global.alignItemsCenter]}>
                <Text style={style.formIcon}></Text>
                <Text style={[style.formThem, global.fontStyle, global.fontSize14]}>
                  选择擅长治疗疾病(最多选择10个)</Text>
              </View>
              {this.state.hospitalDepartmentSymptom.map((v: any, k: number) => {
                return (<View key={k} style={style.symptomList}>
                  ...
                  <View style={[global.flex, global.flexWrap]}>
                    {v.symptomList.map((v1: any, k1: number) => {
                      return (<TouchableOpacity key={k1} onPress={() => {
                        let hospitalDepartmentSymptom =
                          this.state.hospitalDepartmentSymptom,
                          adeptSymptomIdList = this.state.adeptSymptomIdList;
                        if (adeptSymptomIdList.length < 10) {
                          hospitalDepartmentSymptom[k].symptomList[k1]
                        ...
                        <Text style={[v1.isChecked ? style.symptomItemActive :
                          style.symptomItem, global.fontSize14, global.fontStyle]}>
                          {v1.name}</Text>
                      </TouchableOpacity>)
                    })}
                  </View>
